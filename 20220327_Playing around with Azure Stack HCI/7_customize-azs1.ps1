...
Enter-PSSession -VMName "AZS1" -Credential $localAdmin
...
# set IP addresses (AZS1)
$ia = "Corporate"
New-NetIPAddress -InterfaceAlias $ia -IPAddress "192.168.111.51" -PrefixLength "24" -DefaultGateway "192.168.111.1"
...
# set other IP addresses (AZS1)
$ia = "Storage_1"
New-NetIPAddress -InterfaceAlias $ia -IPAddress "10.111.111.1" -PrefixLength "30"
Set-DnsClient -InterfaceAlias $ia -RegisterThisConnectionsAddress $false
$ia = "Storage_2"
New-NetIPAddress -InterfaceAlias $ia -IPAddress "10.111.112.1" -PrefixLength "30"
Set-DnsClient -InterfaceAlias $ia -RegisterThisConnectionsAddress $false
$ia = "VM"
New-NetIPAddress -InterfaceAlias $ia -IPAddress "10.111.113.1" -PrefixLength "30"
Set-DnsClient -InterfaceAlias $ia -RegisterThisConnectionsAddress $false
...
# domain join
Add-Computer -DomainName "lab.tklabs.eu" -Credential "LAB\Administrator" -NewName "AZS1" -Restart -Force

## additional settings
$domainadmin = Get-Credential -UserName "LAB\Administrator" -Message "Enter password for the domain Administrator account:"
Enter-PSSession -VMName "AZS1" -Credential $domainadmin
